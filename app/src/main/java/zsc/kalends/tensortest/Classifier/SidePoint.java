...
public class SidePoint {
    ...
    private static final int POINTS_NUM = 12;
    private static final int CHANNELS_NUM = 3;

    public SidePoint(TensorFlowInferenceInterface infer) {
        this.inferenceInterface = infer;
    }

    public Point[] getResult(Mat image) {
        ...
        Point[] results = new Point[POINTS_NUM];
        int index = 0;
        for (int i = 0; i < POINTS_NUM; i++) {
            ...
            results[i].x = (float) (maxwIndex * image.size().width) / IMAGE_WIDTH;
            results[i].y = (float) (maxhIndex * image.size().height) / IMAGE_HEIGHT;
        }
        return results;
    }
}
